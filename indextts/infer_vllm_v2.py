...
class IndexTTS2:
    ...
    async def infer_stream(
        ...
        audio, sr0 = librosa.load(spk_audio_prompt)
        audio = torch.tensor(audio).unsqueeze(0)
        audio_22k = torchaudio.transforms.Resample(sr0, 22050)(audio)
        audio_16k = torchaudio.transforms.Resample(sr0, 16000)(audio)
        ...
        for sent in sentences:
            ...
            # but instead of wavs.append(wav.cpu()), you yield bytes.
            ...
            with torch.no_grad():
                ...
                # Fix code length exactly like your infer()
                code_lens = []
                for code in codes:
                    if self.stop_mel_token not in code:
                        code_len = len(code)
